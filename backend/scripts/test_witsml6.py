...
last_time = (datetime.utcnow() - timedelta(seconds=30)).strftime('%Y-%m-%dT%H:%M:%SZ')
...
<soap-env:Envelope xmlns:soap-env="http://schemas.xmlsoap.org/soap/envelope/">
  <soap-env:Body>
    <ns0:WMLS_GetFromStore xmlns:ns0="http://www.witsml.org/message/120">
      ...
      <OptionsIn>returnElements=data-only</OptionsIn>
...
print("\\n[Test] Sending exact Zeep payload via raw HTTP POST...")
t0 = time.time()
try:
    s = requests.Session()
    ...
    if 'Result' in r.text or 'WMLS_GetFromStoreResult' in r.text:
        ...
        print("Failed to parse. Status:", r.status_code)
        print(r.text)
