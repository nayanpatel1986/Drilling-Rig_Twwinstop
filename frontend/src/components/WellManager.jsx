...
export default function WellManager() {
    ...
    return (
        <div className="px-4 py-2 bg-gray-900 border-b border-white/10 flex items-center h-[100px] relative">
            ...
            <div className="flex items-center gap-16 px-8 py-1 rounded-2xl border border-white/5 bg-white/[0.02]">
                <div className="flex flex-col items-center min-w-[140px]">
                    <span className="text-[11px] text-gray-400 font-extrabold uppercase tracking-[0.2em] mb-1 flex items-center gap-2">
                        <Gauge size={14} className={activityText} /> ACTIVITY
                    </span>
                    <span className={`text-xl font-black tracking-tighter ${activityText} drop-shadow-[0_0_10px_rgba(56,189,248,0.2)]`}>
                        {rigActivity}
                    </span>
                </div>
                ...
                <div className="flex flex-col items-center min-w-[160px]">
                    <span className="text-[11px] text-gray-400 font-extrabold uppercase tracking-[0.2em] mb-1 flex items-center gap-2">
                        <ArrowDownCircle size={14} className="text-cyan-500" /> HOLE DEPTH
                    </span>
                    <div className="flex items-baseline gap-1.5">
                        <span className="text-3xl font-black text-blue-400 tracking-tighter tabular-nums leading-none">
                            {val('Depth').toFixed(2)}
                        </span>
                        <span className="text-xs text-blue-400/80 font-black">m</span>
                    </div>
                </div>
                ...
                <div className="flex flex-col items-center min-w-[160px]">
                    <span className="text-[11px] text-gray-400 font-extrabold uppercase tracking-[0.2em] mb-1 flex items-center gap-2">
                        <MapPin size={14} className="text-cyan-500" /> BIT POSITION
                    </span>
                    <div className="flex items-baseline gap-1.5">
                        <span className="text-3xl font-black text-blue-400 tracking-tighter tabular-nums leading-none">
            ...
            <div className="flex-1 flex justify-end items-center gap-6">
                <div className="flex items-center gap-4">
                    <span className="text-xs text-gray-500 font-bold uppercase tracking-wider">Active Well:</span>
                    {activeWell ? (
                        <div className="flex items-center gap-2">
                            <span className="font-black text-nov-accent tracking-tight">{activeWell.name}</span>
